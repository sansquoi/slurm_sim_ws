...
def check_slurm_conf(slurm_conf, slurm_etc, slurm_bin_top, slurmdbd_conf):
    ...
    if "PluginDir".lower() in slurm_conf:
        if os.path.abspath(slurm_conf["PluginDir".lower()]) != os.path.abspath(
                os.path.join(slurm_bin_top, "lib", "slurm")):
            error_count += 1
            log.error("PluginDir from slurm.conf (%s) does not match provided slurm binary path(%s)!" % (
                os.path.abspath(slurm_conf["PluginDir".lower()]),
                os.path.abspath(os.path.join(slurm_bin_top, "lib", "slurm"))))

    else:
        error_count += 1
        log.error("PluginDir from slurm.conf is not set, set it to: " + os.path.abspath(
            os.path.join(slurm_bin_top, "lib", "slurm")))
    ...
    if "ControlMachine".lower() in slurm_conf:
        if slurm_conf["ControlMachine".lower()] != "localhost":
            error_count += 1
            log.error("For simulation ControlMachine from slurm.conf should be localhost not %s!" % (
                slurm_conf["ControlMachine".lower()],))
    else:
        error_count += 1
    ...
    if "ControlAddr".lower() in slurm_conf:
        if slurm_conf["ControlAddr".lower()] != "localhost":
            error_count += 1
            log.error("For simulation ControlAddr from slurm.conf should be localhost not %s!" % (
                slurm_conf["ControlAddr".lower()],))
    else:
        error_count += 1
    ...
    if "JobCompType".lower() in slurm_conf:
        if slurm_conf["JobCompType".lower()] != "jobcomp/filesacctout":
            error_count += 1
            log.error(
                "For simulation JobCompType should be set to jobcomp/filesacctout, it provides sacct like output which is easy for loading in R!")
    else:
        error_count += 1
        log.error(
            "For simulation JobCompType should be set to jobcomp/filesacctout,  it provides sacct like output which is easy for loading in R!")
        ...
        if slurm_conf["AccountingStorageType".lower()] == "accounting_storage/slurmdbd":
            if "AccountingStorageHost".lower() not in slurm_conf:
                ...
                if slurm_conf["AccountingStorageHost".lower()] != "localhost":
                    error_count += 1
                    log.error("For simulation AccountingStorageHost (currently set to " + slurm_conf[
                        "AccountingStorageHost".lower()] + ") should be set to localhost!")
    else:
        error_count += 1
    ...
    if "DefaultStoragePort".lower() in slurm_conf and "DbdPort".lower() in slurmdbd_conf:
        if slurm_conf["DefaultStoragePort".lower()] != slurmdbd_conf["DbdPort".lower()]:
            error_count += 1
            log.error("DefaultStoragePort (%s) should match DbdPort (%s) but it does not!" % (
                slurm_conf["DefaultStoragePort".lower()], slurmdbd_conf["DbdPort".lower()]))
...
def check_slurmdbd_conf(slurmdbd_conf, slurm_etc, slurm_bin_top):
    ...
    if "PluginDir".lower() in slurmdbd_conf:
        if os.path.abspath(slurmdbd_conf["PluginDir".lower()]) != os.path.abspath(
                os.path.join(slurm_bin_top, "lib", "slurm")):
            error_count += 1
            log.error("PluginDir from slurm.conf (%s) does not match provided slurm binary path(%s)!" % (
                os.path.abspath(slurmdbd_conf["PluginDir".lower()]),
                os.path.abspath(os.path.join(slurm_bin_top, "lib", "slurm"))))
...
def run_sim(args):
    ...
    if "StorageHost".lower() in slurmdbd_conf and args.delete:
        log.info("truncating db from previous runs")
    ...
    if shared_memory_name is not None:
        ...
        if os.path.exists(filename):
            if args.delete:
                ...
                raise Exception(
                    "shared memory from previous run still on system (" + filename + ").Can not continue simulation." + \
    ...
    start = time()
    if not args.no_slurmctld:
        if args.octld == "":
            # toruncmd = f"gnome-terminal -e '{slurmctld_loc} -Dvvv'" terminal_proc = subprocess.Popen([toruncmd,
            # '-D'], env={'SLURM_CONF': slurm_conf_loc}, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)

            os.environ["SLURM_CONF"] = slurm_conf_loc

            # slurmctld_proc = subprocess.Popen([slurmctld_loc, '-D'])

            # subprocess.call(["gnome-terminal", "-e", slurmctld_loc, "-D"])

            fuzz_proc = subprocess.call(
                ["afl-fuzz", "-i", "/home/slurm/slurm_sim_ws/fuzz/inputs",
                 "-o",
                 "/home/slurm/slurm_sim_ws/fuzz/outputs2", slurmctld_loc])

            # , "@@"
            # fuzz_proc = subprocess.call( ["gnome-terminal", "-e", "AFL_SKIP_CPUFREQ=1 afl-fuzz", "-i",
            # "/home/slurm/slurm_sim_ws/fuzz/inputs", "-o", "/home/slurm/slurm_sim_ws/fuzz/outputs", slurmctld_loc,
            # "@@"])

            # slurmctld_proc = subprocess.Popen([slurmctld_loc, '-D'], stdout=subprocess.DEVNULL,
            ...
            # subprocess.call(["gnome-terminal", "--window-with-profile=fuzz", "-e", "sudo apt update"])

            # afl_proc = subprocess.Popen(["afl-fuzz", "-i", "/home/slurm/slurm_sim_ws/fuzz/inputs", "-o",
            #                              "/home/slurm/slurm_sim_ws/fuzz/outputs", slurmctld_loc, "@@"],
    ...
    if slurmdbd_proc is not None:
        slurmdbd_proc.kill()
        slurmdbd_proc = None

    if slurmctld_proc is not None:
        slurmctld_proc.kill()
        slurmctld_proc = None

    if slurmctld_out is not None:
        slurmctld_out.close()
        slurmctld_out = None
    if slurmdbd_out is not None:
        slurmdbd_out.close()
        slurmdbd_out = None
    ...
    # copy files to result storage directory
